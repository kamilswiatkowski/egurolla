...
class Ai1wm_Cron
{
     ...
     * @return mixed
     */
    public static function add($hook, $recurrence, $timestamp, $args = [])
    {
     ...
     * @return boolean
     */
    public static function exists($hook, $args = [])
    {
        $cron = get_option(AI1WM_CRON, []);
        if (empty($cron)) {
            return false;
     ...
     * @return boolean
     */
    public static function delete($hook, $args = [])
    {
        $cron = get_option(AI1WM_CRON, []);
        if (empty($cron)) {
            return false;
