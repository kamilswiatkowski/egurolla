...
class Ai1wm_Extractor extends Ai1wm_Archiver
{
    ...
    public function list_files()
    {
        ...
        if (@fseek($this->file_handle, 0, SEEK_SET) === -1) {
            throw new Ai1wm_Not_Seekable_Exception(
              sprintf(__('Unable to seek to beginning of file. File: %s', AI1WM_PLUGIN_NAME), $this->file_name)
            );
        }
     ...
     *
     */
    public function extract_one_file_to(
      $location,
      $exclude_files = [],
      $exclude_extensions = [],
      $old_paths = [],
    ...
    )
    {
        if (false === is_dir($location)) {
            throw new Ai1wm_Not_Directory_Exception(
              sprintf(__('Location is not a directory: %s', AI1WM_PLUGIN_NAME), $location)
            );
        }
            ...
            if ($block === $this->eof) {
                ...
                if (($data = $this->get_data_from_block($block))) {
                    ...
                    for ($i = 0; $i < count($exclude_extensions); $i++) {
                        if (strrpos($file_name, $exclude_extensions[$i]) === strlen($file_name) - strlen(
                            $exclude_extensions[$i]
                          )) {
                            $should_exclude_file = true;
                            break;
                        }
                    ...
                    if ($should_exclude_file === false) {
                        ...
                        if (path_is_absolute($file_path)) {
                            ...
                            $file_path = ai1wm_escape_windows_directory_separator(
                              $location . DIRECTORY_SEPARATOR . $file_path
                            );
                        }
                        ...
                        if (path_is_absolute($file_name)) {
                            ...
                            $file_name = ai1wm_escape_windows_directory_separator(
                              $location . DIRECTORY_SEPARATOR . $file_name
                            );
                        }
                ...
                if ($chunk_size > 0) {
                    ...
                    if (($file_content = @fread($this->file_handle, $chunk_size)) === false) {
                        throw new Ai1wm_Not_Readable_Exception(
                          sprintf(__('Unable to read content from file. File: %s', AI1WM_PLUGIN_NAME), $this->file_name)
                        );
                    }
                    ...
                    if ( ! empty($ai1wm_params['decryption_password']) && basename($file_name) !== 'package.json') {
                        $file_content = ai1wm_decrypt_string(
                          $file_content,
                          $ai1wm_params['decryption_password'],
                          $file_name
                        );
                    }
                    ...
                    if (($file_bytes = @fwrite($file_handle, $file_content)) !== false) {
                        if (strlen($file_content) !== $file_bytes) {
                            throw new Ai1wm_Quota_Exceeded_Exception(
                              sprintf(
                                __('Out of disk space. Unable to write content to file. File: %s', AI1WM_PLUGIN_NAME),
                                $file_name
                              )
                            );
                        }
