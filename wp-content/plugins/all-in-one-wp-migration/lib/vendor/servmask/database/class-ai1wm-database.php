...
abstract class Ai1wm_Database
{
     ...
     * @var array
     */
    protected $reserved_column_prefixes = [];
     ...
     * @var array
     */
    protected $old_replace_raw_values = [];
     ...
     * @var array
     */
    protected $new_replace_raw_values = [];
     ...
     * @var array
     */
    protected $table_select_columns = [];
     ...
     * @var array
     */
    protected $table_prefix_filters = [];
    ...
    {
        $this->table_prefix_filters[] = [$table_prefix, $exclude_prefix];
                ...
                if (in_array($table_name, $views)) {
                    ...
                    do {
                        ...
                        if ($primary_keys) {
                            ...
                            $query = sprintf(
                              'SELECT %s FROM `%s` AS t1 JOIN (SELECT %s FROM `%s` WHERE %s ORDER BY %s LIMIT %d, %d) AS t2 USING (%s)',
                              $select_columns,
                              $table_name,
                              $table_keys,
                              $table_name,
                    ...
                    if (strlen($query) <= $max_allowed_packet) {
                        ...
                        if ( ! $this->is_cache_query($query)) {
                            ...
                            if ($this->errno() === 1813) {
                                throw new Ai1wm_Database_Exception(
                                  __(
                                    'Error importing database table. <a href="https://help.servmask.com/knowledgebase/mysql-error-importing-table/" target="_blank">Technical details</a>',
                                    AI1WM_PLUGIN_NAME
                                  ), 503
                                );
                            }
    ...
    protected function replace_table_values($input)
    {
        ...
        if ($this->get_visual_composer()) {
            $input = preg_replace_callback(
              ...
              [$this, 'replace_visual_composer_values_callback'],
              $input
            );
        }
        ...
        if ($this->get_oxygen_builder()) {
            $input = preg_replace_callback(
              ...
              [$this, 'replace_oxygen_builder_values_callback'],
              $input
            );
        }
        ...
        if ($this->get_betheme_responsive() || $this->get_optimize_press() || $this->get_avada_fusion_builder()) {
            $input = preg_replace_callback(
              "/'([a-zA-Z0-9\/+]+={0,2})'/S",
              [$this, 'replace_base64_values_callback'],
              $input
            );
        }
        ...
        foreach ($this->get_old_replace_values() as $old_value) {
            if (strpos($input, $this->escape($old_value)) !== false) {
                $input = preg_replace_callback("/'(.*?)(?<!\\\\)'/S", [$this, 'replace_table_values_callback'], $input);
                break;
            }
        }
    ...
    {
        return Ai1wm_Database_Utility::replace_values(
          $this->get_old_replace_raw_values(),
          $this->get_new_replace_raw_values(),
          $input
        );
    }
    ...
    protected function replace_visual_composer_values_callback($matches)
    {
        ...
        if (Ai1wm_Database_Utility::base64_validate($matches[1])) {
            ...
            // Replace values
            $matches[1] = Ai1wm_Database_Utility::replace_values(
              $this->get_old_replace_values(),
              $this->get_new_replace_values(),
              $matches[1]
            );
    ...
    protected function replace_oxygen_builder_values_callback($matches)
    {
        ...
        if (Ai1wm_Database_Utility::base64_validate($matches[2])) {
            ...
            // Replace values
            $matches[2] = Ai1wm_Database_Utility::replace_values(
              $this->get_old_replace_values(),
              $this->get_new_replace_values(),
              $matches[2]
            );
    ...
    protected function replace_base64_values_callback($matches)
    {
        ...
        if (Ai1wm_Database_Utility::base64_validate($matches[1])) {
            ...
            // Replace serialized values
            $matches[1] = Ai1wm_Database_Utility::replace_serialized_values(
              $this->get_old_replace_values(),
              $this->get_new_replace_values(),
              $matches[1]
            );
