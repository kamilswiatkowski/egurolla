...
class Ai1wm_Main_Controller
{
    ...
    {
        register_activation_hook(AI1WM_PLUGIN_BASENAME, [$this, 'activation_hook']);
    ...
    private function activate_actions()
    {
        ...
        // Enable WP importing
        add_action('admin_init', [$this, 'wp_importing'], 5);

        // Setup backups folder
        add_action('admin_init', [$this, 'setup_backups_folder']);

        // Setup storage folder
        add_action('admin_init', [$this, 'setup_storage_folder']);

        // Setup secret key
        add_action('admin_init', [$this, 'setup_secret_key']);

        // Check user role capability
        add_action('admin_init', [$this, 'check_user_role_capability']);
        ...
        add_action('plugins_loaded', [$this, 'ai1wm_loaded'], 10);

        // Export and import commands
        add_action('plugins_loaded', [$this, 'ai1wm_commands'], 10);

        // Export and import buttons
        add_action('plugins_loaded', [$this, 'ai1wm_buttons'], 10);

        // WP CLI commands
        add_action('plugins_loaded', [$this, 'wp_cli'], 10);

        // Register scripts and styles
        add_action('admin_enqueue_scripts', [$this, 'register_scripts_and_styles'], 5);

        // Enqueue export scripts and styles
        add_action('admin_enqueue_scripts', [$this, 'enqueue_export_scripts_and_styles'], 5);

        // Enqueue import scripts and styles
        add_action('admin_enqueue_scripts', [$this, 'enqueue_import_scripts_and_styles'], 5);

        // Enqueue backups scripts and styles
        add_action('admin_enqueue_scripts', [$this, 'enqueue_backups_scripts_and_styles'], 5);

        // Enqueue what's new scripts and styles
        add_action('admin_enqueue_scripts', [$this, 'enqueue_whats_new_scripts_and_styles'], 5);

        // Enqueue updater scripts and styles
        add_action('admin_enqueue_scripts', [$this, 'enqueue_updater_scripts_and_styles'], 5);
    }
    ...
    private function activate_filters()
    {
        ...
        add_filter('cron_schedules', [$this, 'add_cron_schedules'], 9999);
    }
    ...
    public function create_litespeed_htaccess($path)
    {
        if ( ! Ai1wm_File_Htaccess::litespeed($path)) {
            if (is_multisite()) {
                return add_action('network_admin_notices', [$this, 'wordpress_htaccess_notice']);
            } else {
                return add_action('admin_notices', [$this, 'wordpress_htaccess_notice']);
            }
        }
    }
    ...
    public function create_backups_folder($path)
    {
        if ( ! Ai1wm_Directory::create($path)) {
            if (is_multisite()) {
                return add_action('network_admin_notices', [$this, 'backups_path_notice']);
            } else {
                return add_action('admin_notices', [$this, 'backups_path_notice']);
            }
        }
    }
    ...
    public function create_backups_htaccess($path)
    {
        if ( ! Ai1wm_File_Htaccess::create($path)) {
            if (is_multisite()) {
                return add_action('network_admin_notices', [$this, 'backups_htaccess_notice']);
            } else {
                return add_action('admin_notices', [$this, 'backups_htaccess_notice']);
            }
        }
    }
    ...
    public function create_backups_webconfig($path)
    {
        if ( ! Ai1wm_File_Webconfig::create($path)) {
            if (is_multisite()) {
                return add_action('network_admin_notices', [$this, 'backups_webconfig_notice']);
            } else {
                return add_action('admin_notices', [$this, 'backups_webconfig_notice']);
            }
        }
    }
    ...
    public function create_backups_index_php($path)
    {
        if ( ! Ai1wm_File_Index::create($path)) {
            if (is_multisite()) {
                return add_action('network_admin_notices', [$this, 'backups_index_php_notice']);
            } else {
                return add_action('admin_notices', [$this, 'backups_index_php_notice']);
            }
        }
    }
    ...
    public function create_backups_index_html($path)
    {
        if ( ! Ai1wm_File_Index::create($path)) {
            if (is_multisite()) {
                return add_action('network_admin_notices', [$this, 'backups_index_html_notice']);
            } else {
                return add_action('admin_notices', [$this, 'backups_index_html_notice']);
            }
        }
    }
    ...
    public function create_backups_robots_txt($path)
    {
        if ( ! Ai1wm_File_Robots::create($path)) {
            if (is_multisite()) {
                return add_action('network_admin_notices', [$this, 'backups_robots_txt_notice']);
            } else {
                return add_action('admin_notices', [$this, 'backups_robots_txt_notice']);
            }
        }
    }
    ...
    public function create_storage_folder($path)
    {
        if ( ! Ai1wm_Directory::create($path)) {
            if (is_multisite()) {
                return add_action('network_admin_notices', [$this, 'storage_path_notice']);
            } else {
                return add_action('admin_notices', [$this, 'storage_path_notice']);
            }
        }
    }
    ...
    public function create_storage_index_php($path)
    {
        if ( ! Ai1wm_File_Index::create($path)) {
            if (is_multisite()) {
                return add_action('network_admin_notices', [$this, 'storage_index_php_notice']);
            } else {
                return add_action('admin_notices', [$this, 'storage_index_php_notice']);
            }
        }
    }
    ...
    public function create_storage_index_html($path)
    {
        if ( ! Ai1wm_File_Index::create($path)) {
            if (is_multisite()) {
                return add_action('network_admin_notices', [$this, 'storage_index_html_notice']);
            } else {
                return add_action('admin_notices', [$this, 'storage_index_html_notice']);
            }
        }
    }
    ...
    public function ai1wm_loaded()
    {
        ...
        foreach (Ai1wm_Extensions::get() as $slug => $extension) {
            add_action(
              "in_plugin_update_message-{$extension['basename']}",
    ...
    public function wp_cli()
    {
        if (defined('WP_CLI')) {
            WP_CLI::add_command(
              'ai1wm',
              'Ai1wm_WP_CLI_Command',
              ['shortdesc' => __('All-in-One WP Migration Command', AI1WM_PLUGIN_NAME)]
            );
        }
    }
    ...
    public function check_user_role_capability()
    {
        if (($user = wp_get_current_user()) && in_array('administrator', $user->roles)) {
            if ( ! $user->has_cap('export') || ! $user->has_cap('import')) {
                if (is_multisite()) {
                    return add_action('network_admin_notices', [$this, 'missing_role_capability_notice']);
                } else {
                    return add_action('admin_notices', [$this, 'missing_role_capability_notice']);
                }
            }
        }
    }
    ...
    public function admin_menu()
    {
        ...
        add_submenu_page(
          ...
          __('Backups', AI1WM_PLUGIN_NAME) . Ai1wm_Template::get_content(
            'main/backups',
            ['count' => Ai1wm_Backups::count_files()]
          ),
          'import',
          'ai1wm_backups',
        ...
        add_submenu_page(
          ...
          __('What\'s new', AI1WM_PLUGIN_NAME) . Ai1wm_Template::get_content('main/whats-new', []),
          'import',
          'ai1wm_whats_new',
    ...
    public function enqueue_export_scripts_and_styles($hook)
    {
          ...
          [
            'ajax'       => [
              'url' => wp_make_link_relative(
                add_query_arg(['ai1wm_import' => 1], admin_url('admin-ajax.php?action=ai1wm_export'))
              ),
            ],
            'status'     => [
              'url' => wp_make_link_relative(
                add_query_arg(['ai1wm_import' => 1, 'secret_key' => get_option(AI1WM_SECRET_KEY)],
        ...
        wp_localize_script(
          ...
          'ai1wm_locale',
          [
            'stop_exporting_your_website'         => __(
              'You are about to stop exporting your website, are you sure?',
              AI1WM_PLUGIN_NAME
            ),
            'preparing_to_export'                 => __('Preparing to export...', AI1WM_PLUGIN_NAME),
            'unable_to_export'                    => __('Unable to export', AI1WM_PLUGIN_NAME),
            'unable_to_start_the_export'          => __(
              'Unable to start the export. Refresh the page and try again',
              AI1WM_PLUGIN_NAME
            ),
            'unable_to_run_the_export'            => __(
              'Unable to run the export. Refresh the page and try again',
              AI1WM_PLUGIN_NAME
            ),
            'unable_to_stop_the_export'           => __(
              'Unable to stop the export. Refresh the page and try again',
              AI1WM_PLUGIN_NAME
            ),
    ...
    public function enqueue_import_scripts_and_styles($hook)
    {
        ...
        wp_localize_script(
          ...
          [
            ...
            'url'           => wp_make_link_relative(
              add_query_arg(['ai1wm_import' => 1], admin_url('admin-ajax.php?action=ai1wm_import'))
            ),
            'params'        => [
              'priority'   => 5,
              'secret_key' => get_option(AI1WM_SECRET_KEY),
        ...
        wp_localize_script(
          ...
          'ai1wm_compatibility',
          [
            'messages' => Ai1wm_Compatibility::get([]),
          ]
        );
        ...
        wp_localize_script(
          ...
          'ai1wm_locale',
          [
            'stop_importing_your_website'         => __(
              'You are about to stop importing your website, are you sure?',
              AI1WM_PLUGIN_NAME
            ),
            'preparing_to_import'                 => __('Preparing to import...', AI1WM_PLUGIN_NAME),
            'unable_to_import'                    => __('Unable to import', AI1WM_PLUGIN_NAME),
            'unable_to_start_the_import'          => __(
              'Unable to start the import. Refresh the page and try again',
              AI1WM_PLUGIN_NAME
            ),
            'unable_to_confirm_the_import'        => __(
              'Unable to confirm the import. Refresh the page and try again',
              AI1WM_PLUGIN_NAME
            ),
            'unable_to_check_decryption_password' => __(
              'Unable to check decryption password. Refresh the page and try again',
              AI1WM_PLUGIN_NAME
            ),
            'unable_to_prepare_blogs_on_import'   => __(
              'Unable to prepare blogs on import. Refresh the page and try again',
              AI1WM_PLUGIN_NAME
            ),
            'unable_to_stop_the_import'           => __(
              'Unable to stop the import. Refresh the page and try again',
              AI1WM_PLUGIN_NAME
            ),
            ...
            'please_do_not_close_this_browser'    => __(
            ...
            'backup_encrypted_message'            => __(
              'Please enter a password to import the file',
              AI1WM_PLUGIN_NAME
            ),
    ...
    public function enqueue_backups_scripts_and_styles($hook)
    {
        ...
        wp_localize_script(
          ...
          [
            'ajax'       => [
              'url' => wp_make_link_relative(
                add_query_arg(['ai1wm_import' => 1], admin_url('admin-ajax.php?action=ai1wm_backup_list_content'))
              ),
            ],
            'download'   => [
              'url' => wp_make_link_relative(
                add_query_arg(['ai1wm_import' => 1], admin_url('admin-ajax.php?action=ai1wm_backup_download_file'))
              ),
            ],
            'secret_key' => get_option(AI1WM_SECRET_KEY),
          ]
        ...
        wp_localize_script(
          ...
          [
            ...
            'thanks_for_submitting_your_feedback' => __('Thanks for submitting your feedback!', AI1WM_PLUGIN_NAME),
            'thanks_for_submitting_your_request'  => __('Thanks for submitting your request!', AI1WM_PLUGIN_NAME),
            'want_to_delete_this_file'            => __(
              'Are you sure you want to delete this file?',
              AI1WM_PLUGIN_NAME
            ),
            'unlimited'                           => __(
              'Restoring a backup is available via Unlimited extension. <a href="https://servmask.com/products/unlimited-extension" target="_blank">Get it here</a>',
              AI1WM_PLUGIN_NAME
            ),
            'restore_from_file'                   => __(
    ...
    public function enqueue_updater_scripts_and_styles($hook)
    {
        ...
        wp_localize_script(
          ...
          [
            'ajax' => [
              'url' => wp_make_link_relative(
                add_query_arg(['ai1wm_nonce' => wp_create_nonce('ai1wm_updater')],
        ...
        wp_localize_script(
          ...
          [
            ...
            'invalid_purchase_id' => __(
              'Your purchase ID is invalid, please <a href="mailto:support@servmask.com">contact us</a>',
              AI1WM_PLUGIN_NAME
            ),
          ]
        );
    ...
    public function admin_head()
    {
        ...
        // Admin header
        Ai1wm_Template::render('main/admin-head', ['version' => $wp_version]);
    }
