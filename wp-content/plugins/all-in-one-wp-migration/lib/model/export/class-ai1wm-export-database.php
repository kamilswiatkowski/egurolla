...
class Ai1wm_Export_Database
{
    ...
    public static function execute($params)
    {
        ...
        Ai1wm_Status::info(
          sprintf(
            __('Exporting database...<br />%d%% complete<br />%s records saved', AI1WM_PLUGIN_NAME),
            $progress,
        ...
        if (isset($params['options']['no_spam_comments'])) {
            $mysql->set_table_where_query(ai1wm_table_prefix() . 'comments', "`comment_approved` != 'spam'")
              ->set_table_where_query(
                ai1wm_table_prefix() . 'commentmeta',
                sprintf(
                  "`comment_ID` IN ( SELECT `comment_ID` FROM `%s` WHERE `comment_approved` != 'spam' )",
        ...
        $old_table_prefixes = $old_column_prefixes = [];
        $new_table_prefixes = $new_column_prefixes = [];
        ...
        $mysql->set_reserved_column_prefixes(['wp_force_deactivated_plugins', 'wp_page_for_privacy_policy']);
        ...
        if (($column_names = $mysql->get_column_names(ai1wm_table_prefix() . 'options'))) {
            if (isset($column_names['option_name'], $column_names['option_value'])) {
                $column_names['option_value'] = sprintf(
                  "(CASE WHEN option_name = '%s' THEN 'a:0:{}' WHEN (option_name = '%s' OR option_name = '%s') THEN '' ELSE option_value END) AS option_value",
        ...
        $mysql->set_table_prefix_columns(ai1wm_table_prefix() . 'options', ['option_name'])
          ->set_table_prefix_columns(ai1wm_table_prefix() . 'usermeta', ['meta_key']);
