...
class Ai1wm_Export_Enumerate_Content
{
    ...
    {
        $exclude_filters = array_merge([ai1wm_get_uploads_dir(), ai1wm_get_plugins_dir()], ai1wm_get_themes_dirs());
        ...
        if (isset($params['options']['no_themes'], $params['options']['no_muplugins'], $params['options']['no_plugins']) === false) {
            ...
            $iterator = new Ai1wm_Recursive_Exclude_Filter(
              $iterator,
              apply_filters(
                'ai1wm_exclude_content_from_export',
                ai1wm_content_filters($exclude_filters)
              )
            );
