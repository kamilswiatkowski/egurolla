...
class Ai1wm_Backups
{
    ...
    public static function get_files()
    {
        ...
        try {
            ...
            $iterator = new Ai1wm_Recursive_Extension_Filter($iterator, ['wpress']);
    ...
    public static function delete_file($file)
    {
        if (ai1wm_is_filename_supported($file)) {
            return @unlink(ai1wm_backup_path(['archive' => $file]));
        }
    }
    ...
    {
        return get_option(AI1WM_BACKUPS_LABELS, []);
    }
    ...
    {
        if (($labels = get_option(AI1WM_BACKUPS_LABELS, [])) !== false) {
            $labels[$file] = $label;
        }
    ...
    public static function are_downloadable()
    {
        ...
        if (is_null($downloadable)) {
            $downloadable = Ai1wm_Backups::are_in_wp_content_folder() || strpos(
                                                                           AI1WM_BACKUPS_PATH,
