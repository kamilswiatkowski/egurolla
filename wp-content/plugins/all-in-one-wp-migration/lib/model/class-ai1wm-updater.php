...
class Ai1wm_Updater
{
    ...
    public static function plugins_api($result, $action = null, $args = null)
    {
        ...
        if (isset($extensions[$args->slug]) && $action === 'plugin_information') {
            $updater = get_option(AI1WM_UPDATER, []);
            ...
            if (isset($extensions[$slug], $update['version'], $update['homepage'], $update['download_link'], $update['icons'])) {
                if (($purchase_id = get_option($extensions[$slug]['key']))) {
                    ...
                    if ($slug === 'all-in-one-wp-migration-file-extension') {
                        $download_url = add_query_arg(['siteurl' => get_site_url()],
                                                      sprintf('%s', $update['download_link']));
                    } else {
                        $download_url = add_query_arg(['siteurl' => get_site_url()],
                                                      sprintf('%s/%s', $update['download_link'], $purchase_id));
                    }
            ...
            if (is_wp_error($about)) {
                ...
                if ($slug !== 'all-in-one-wp-migration-file-extension') {
                    if (($purchase_id = get_option($extension['key']))) {
                        $check = wp_remote_get(
                          add_query_arg(['site_url' => get_site_url(), 'admin_email' => get_option('admin_email')],
                                        sprintf('%s/%s', $extension['check'], $purchase_id)),
                          [
                            'timeout' => 15,
                            'headers' => ['Accept' => 'application/json'],
                          ]
        ...
        foreach (Ai1wm_Extensions::get() as $slug => $extension) {
            ...
            if ($plugin_file === $extension['basename']) {
                ...
                $updater_url = add_query_arg(
                  ['ai1wm_check_for_updates' => 1, 'ai1wm_nonce' => wp_create_nonce('ai1wm_check_for_updates')],
                  network_admin_url('plugins.php')
                );

                // Check purchase ID
                if (get_option($extension['key'])) {
                    $plugin_meta[] = Ai1wm_Template::get_content('updater/check', ['url' => $updater_url]);
                } else {
                    $plugin_meta[] = Ai1wm_Template::get_content(
                      'updater/modal',
                      ['url' => $updater_url, 'modal' => $modal_index]
                    );
                }
                // Check error message
                if (isset($updater[$slug]['error_message'])) {
                    $plugin_meta[] = Ai1wm_Template::get_content(
                      'updater/error',
                      ['message' => $updater[$slug]['error_message']]
                    );
                }
            }
        }
