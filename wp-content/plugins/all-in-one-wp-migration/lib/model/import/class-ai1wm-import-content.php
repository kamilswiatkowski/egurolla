...
class Ai1wm_Import_Content
{
    ...
    public static function execute($params)
    {
        ...
        // Set progress
        Ai1wm_Status::info(
          sprintf(__('Restoring %d files...<br />%d%% complete', AI1WM_PLUGIN_NAME), $total_files_count, $progress)
        );
        ...
        $old_paths = ['plugins', 'themes'];
        $new_paths = [ai1wm_get_plugins_dir(), get_theme_root()];
        ...
        while ($archive->has_not_reached_eof()) {
            ...
            // Exclude theme files
            $exclude_files = array_merge($exclude_files, [AI1WM_THEMES_FUNCTIONS_NAME]);

            // Exclude Elementor files
            $exclude_files = array_merge($exclude_files, [AI1WM_ELEMENTOR_CSS_NAME]);

            // Exclude content extensions
            $exclude_extensions = [AI1WM_LESS_CACHE_NAME];

            // Extract a file from archive to WP_CONTENT_DIR
            if (($completed = $archive->extract_one_file_to(
              WP_CONTENT_DIR,
              $exclude_files,
              $exclude_extensions,
              $old_paths,
