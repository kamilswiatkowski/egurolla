...
class Ai1wm_Import_Validate
{
    ...
    public static function execute($params)
    {
        ...
        if ( ! $archive->is_valid()) {
            throw new Ai1wm_Import_Exception(
              __(
                'The archive file is corrupted. Follow <a href="https://help.servmask.com/knowledgebase/corrupted-archive/" target="_blank">this article</a> to resolve the problem.',
                AI1WM_PLUGIN_NAME
              )
            );
        }
        ...
        if ($archive->has_not_reached_eof()) {
            ...
            if (($completed = $archive->extract_by_files_array(
        ...
        if ($archive->has_reached_eof()) {
            ...
            if (false === is_file(ai1wm_package_path($params))) {
                throw new Ai1wm_Import_Exception(
                  __(
                    'Please make sure that your file was exported using <strong>All-in-One WP Migration</strong> plugin. <a href="https://help.servmask.com/knowledgebase/invalid-backup-file/" target="_blank">Technical details</a>',
                    AI1WM_PLUGIN_NAME
                  )
                );
            }
