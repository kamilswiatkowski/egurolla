    ...
    public static function execute($params)
    {
        ...
        switch ($error) {
            case UPLOAD_ERR_OK:
                try {
                    ...
                    throw new Ai1wm_Import_Retry_Exception(
                      sprintf(__('Unable to upload the file because %s', AI1WM_PLUGIN_NAME), $e->getMessage()), 400
                    );
                }
                break;
            ...
            case UPLOAD_ERR_EXTENSION:
                throw new Ai1wm_Import_Retry_Exception(
                  __('A PHP extension stopped the file upload.', AI1WM_PLUGIN_NAME), 400
                );

            default:
                throw new Ai1wm_Import_Retry_Exception(
                  sprintf(__('Unrecognized error %s during upload.', AI1WM_PLUGIN_NAME), $error), 400
                );
        }

        ai1wm_json_response(['errors' => []]);
        exit;
    }
