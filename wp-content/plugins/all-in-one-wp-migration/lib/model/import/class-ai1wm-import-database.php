...
class Ai1wm_Import_Database
{
    ...
    public static function execute($params, Ai1wm_Database $mysql = null)
    {
        ...
        $old_replace_values = $old_replace_raw_values = [];
        $new_replace_values = $new_replace_raw_values = [];
        ...
        foreach ($blogs as $blog) {
            ...
            if ( ! empty($blog['Old']['InternalSiteURL'])) {
                if (parse_url($blog['Old']['InternalSiteURL'], PHP_URL_SCHEME) && parse_url(
                    $blog['Old']['InternalSiteURL'],
                    PHP_URL_HOST
                  )) {
            ...
            foreach ($site_urls as $site_url) {
                ...
                // Replace Site URL
                foreach ([$site_url, $site_url_www_inversion] as $url) {
                    // Get domain
                    ...
                    if ( ! in_array(
                      sprintf("='%s%s", $old_domain, untrailingslashit($old_path)),
                      $old_replace_values
                    )) {
                    ...
                    // Add Site URL scheme
                    $old_schemes = ['http', 'https', ''];
                    $new_schemes = [$new_scheme, $new_scheme, ''];
                    ...
                    for ($i = 0; $i < count($old_schemes); $i++) {
                        ...
                        if ( ! defined('UPLOADBLOGSDIR')) {
                            ...
                            if ( ! in_array(
                              ai1wm_url_scheme(sprintf('%s/files/', untrailingslashit($url)), $old_schemes[$i]),
                              $old_replace_values
                            )) {
                                $old_replace_values[] = ai1wm_url_scheme(
                                  sprintf('%s/files/', untrailingslashit($url)),
                                  $old_schemes[$i]
                                );
                                $new_replace_values[] = ai1wm_url_scheme(
                                  $blog['New']['WordPress']['UploadsURL'],
                                  $new_schemes[$i]
                                );
                            }
                            ...
                            if ( ! in_array(
                              ...
                              $old_replace_values
                                ...
                                $new_replace_values[] = urlencode(
                                  ai1wm_url_scheme($blog['New']['WordPress']['UploadsURL'], $new_schemes[$i])
                                );
                            }
                        ...
                        // Add plain Site URL
                        if ( ! in_array(
                          ai1wm_url_scheme(untrailingslashit($url), $old_schemes[$i]),
                          $old_replace_values
                        )) {
                        ...
                        if ( ! in_array(
                          urlencode(ai1wm_url_scheme(untrailingslashit($url), $old_schemes[$i])),
                          $old_replace_values
                        )) {
                            $old_replace_values[] = urlencode(
                              ai1wm_url_scheme(untrailingslashit($url), $old_schemes[$i])
                            ...
                            $new_replace_values[] = urlencode(
                              ai1wm_url_scheme(untrailingslashit($blog['New']['SiteURL']), $new_schemes[$i])
                            );
                        }

                        // Add URL raw encoded Site URL
                        if ( ! in_array(
                          rawurlencode(ai1wm_url_scheme(untrailingslashit($url), $old_schemes[$i])),
                          $old_replace_values
                        )) {
                            $old_replace_values[] = rawurlencode(
                              ai1wm_url_scheme(untrailingslashit($url), $old_schemes[$i])
                        ...
                        if ( ! in_array(
                          addcslashes(ai1wm_url_scheme(untrailingslashit($url), $old_schemes[$i]), '/'),
                          $old_replace_values
                        )) {
                            $old_replace_values[] = addcslashes(
                              ai1wm_url_scheme(untrailingslashit($url), $old_schemes[$i]),
            ...
            if ( ! empty($blog['Old']['InternalHomeURL'])) {
                if (parse_url($blog['Old']['InternalHomeURL'], PHP_URL_SCHEME) && parse_url(
                    $blog['Old']['InternalHomeURL'],
                    PHP_URL_HOST
                  )) {
            ...
            foreach ($home_urls as $home_url) {
                ...
                // Replace Home URL
                foreach ([$home_url, $home_url_www_inversion] as $url) {
                    // Get domain
                    $old_domain = parse_url($url, PHP_URL_HOST);
                    ...
                    for ($i = 0; $i < count($old_schemes); $i++) {
                        ...
                        if ( ! in_array(
                          ...
                          $old_replace_values
                            ...
                            $old_replace_values[] = ai1wm_url_scheme(untrailingslashit($url), $old_schemes[$i]);
                            $new_replace_values[] = ai1wm_url_scheme(
                              untrailingslashit($blog['New']['HomeURL']),
                              $new_schemes[$i]
                            );
                        }
                        ...
                        if ( ! in_array(
                          ...
                          $old_replace_values
                            ...
                            $new_replace_values[] = urlencode(
                              ai1wm_url_scheme(untrailingslashit($blog['New']['HomeURL']), $new_schemes[$i])
                            );
                        }
            ...
            foreach ($uploads_urls as $uploads_url) {
                ...
                // Replace Uploads URL
                foreach ([$uploads_url, $uploads_url_www_inversion] as $url) {
                    // Get path
                    $old_path = parse_url($url, PHP_URL_PATH);
                    ...
                    for ($i = 0; $i < count($old_schemes); $i++) {
                        ...
                        // Add JSON escaped Uploads URL
                        if ( ! in_array(
                          addcslashes(ai1wm_url_scheme($url, $old_schemes[$i]), '/'),
                          $old_replace_values
                        )) {
        ...
        if ( ! empty($config['InternalSiteURL'])) {
            if (parse_url($config['InternalSiteURL'], PHP_URL_SCHEME) && parse_url(
                ...
                for ($i = 0; $i < count($old_schemes); $i++) {
                    ...
                    if ( ! in_array(
                      ...
                      $old_replace_values
                        ...
                        $new_replace_values[] = urlencode(
                          ai1wm_url_scheme(untrailingslashit(site_url()), $new_schemes[$i])
                        );
                    }
        ...
        if ( ! empty($config['InternalHomeURL'])) {
            if (parse_url($config['InternalHomeURL'], PHP_URL_SCHEME) && parse_url(
                ...
                for ($i = 0; $i < count($old_schemes); $i++) {
                    ...
                    if ( ! in_array(
                      ...
                      $old_replace_values
                        ...
                        $old_replace_values[] = urlencode(ai1wm_url_scheme(untrailingslashit($url), $old_schemes[$i]));
                        $new_replace_values[] = urlencode(
                          ai1wm_url_scheme(untrailingslashit(home_url()), $new_schemes[$i])
                        );
                    }
            ...
            foreach ([$uploads_url, $uploads_url_www_inversion] as $url) {
                ...
                for ($i = 0; $i < count($old_schemes); $i++) {
                    ...
                    if ( ! in_array(rawurlencode(ai1wm_url_scheme($url, $old_schemes[$i])), $old_replace_values)) {
                        $old_replace_values[] = rawurlencode(ai1wm_url_scheme($url, $old_schemes[$i]));
                        $new_replace_values[] = rawurlencode(
                          ai1wm_url_scheme(ai1wm_get_uploads_url(), $new_schemes[$i])
                        );
                    }
        ...
        // Get backups labels
        $backups_labels = get_option(AI1WM_BACKUPS_LABELS, []);

        // Get sites links
        $sites_links = get_option(AI1WM_SITES_LINKS, []);
        ...
        if (ai1wm_validate_plugin_basename('buddyboss-platform/bp-loader.php') || ai1wm_validate_plugin_basename(
        ...
        $mysql->set_atomic_tables([ai1wm_table_prefix() . 'options']);
        ...
        update_option(AI1WM_ACTIVE_PLUGINS, []);
