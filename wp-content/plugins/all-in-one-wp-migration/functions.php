...
function ai1wm_storage_path($params)
{
    if (empty($params['storage'])) {
        throw new Ai1wm_Storage_Exception(
          __(
            'Unable to locate storage path. <a href="https://help.servmask.com/knowledgebase/invalid-storage-path/" target="_blank">Technical details</a>',
            AI1WM_PLUGIN_NAME
          )
        );
    }
...
function ai1wm_backup_path($params)
{
    if (empty($params['archive'])) {
        throw new Ai1wm_Archive_Exception(
          __(
            'Unable to locate archive path. <a href="https://help.servmask.com/knowledgebase/invalid-archive-path/" target="_blank">Technical details</a>',
            AI1WM_PLUGIN_NAME
          )
        );
    }
 ...
 * @return integer
 */
function ai1wm_validate_file($file, $allowed_files = [])
{
 ...
 * @return string
 */
function ai1wm_generate_random_string(
  $length = 12,
  $mixed_chars = true,
  $special_chars = false,
 ...
 * @return array
 */
function ai1wm_content_filters($filters = [])
{
    return array_merge(
      $filters,
      [
 ...
 * @return array
 */
function ai1wm_media_filters($filters = [])
{
    return array_merge(
      $filters,
      [
 ...
 * @return array
 */
function ai1wm_plugin_filters($filters = [])
{
    return array_merge(
      $filters,
      [
 ...
 * @return array
 */
function ai1wm_theme_filters($filters = [])
{
    return array_merge(
      $filters,
      [
 ...
 * @return array
 */
function ai1wm_active_servmask_plugins($plugins = [])
{
    // WP Migration Plugin
...
{
    return array_keys(get_site_option(AI1WM_ACTIVE_SITEWIDE_PLUGINS, []));
}
...
{
    return array_values(get_option(AI1WM_ACTIVE_PLUGINS, []));
}
...
{
    $current = get_site_option(AI1WM_ACTIVE_SITEWIDE_PLUGINS, []);
...
{
    $current = get_option(AI1WM_ACTIVE_PLUGINS, []);
...
{
    $current = get_option(AI1WM_JETPACK_ACTIVE_MODULES, []);
...
{
    $current = get_option(AI1WM_SWIFT_OPTIMIZER_PLUGIN_ORGANIZER, []);
    ...
    if (($plugins = get_plugins())) {
        if (isset($plugins[$basename]['Version']) && ($version = $plugins[$basename]['Version'])) {
            if (version_compare(PHP_VERSION, '7.3', '>=') && version_compare($version, '5.4.8.3', '<')) {
                return ai1wm_deactivate_sitewide_plugins([$basename]);
            }
...
function ai1wm_deactivate_revolution_slider($basename)
{
    if (($plugins = get_plugins())) {
        if (isset($plugins[$basename]['Version']) && ($version = $plugins[$basename]['Version'])) {
            if (version_compare(PHP_VERSION, '7.3', '>=') && version_compare($version, '5.4.8.3', '<')) {
                return ai1wm_deactivate_plugins([$basename]);
            }
...
function ai1wm_cache_flush()
{
    ...
    wp_cache_set('alloptions', [], 'options');
    wp_cache_set('notoptions', [], 'options');

    // Reset WP sitemeta cache
    wp_cache_set('1:notoptions', [], 'site-options');
...
function ai1wm_open($file, $mode)
{
    ...
    if (false === $file_handle) {
        throw new Ai1wm_Not_Accessible_Exception(
          sprintf(
            __(
              'Unable to open %s with mode %s. <a href="https://help.servmask.com/knowledgebase/invalid-file-permissions/" target="_blank">Technical details</a>',
              AI1WM_PLUGIN_NAME
            ),
            $file,
            $mode
    ...
    if (false === $write_result) {
        if (($meta = stream_get_meta_data($handle))) {
            throw new Ai1wm_Not_Writable_Exception(
              sprintf(
                __(
                  'Unable to write to: %s. <a href="https://help.servmask.com/knowledgebase/invalid-file-permissions/" target="_blank">Technical details</a>',
                  AI1WM_PLUGIN_NAME
                ),
                $meta['uri']
              )
        ...
        if (($meta = stream_get_meta_data($handle))) {
            throw new Ai1wm_Quota_Exceeded_Exception(
              sprintf(
                __(
                  'Out of disk space. Unable to write to: %s. <a href="https://help.servmask.com/knowledgebase/out-of-disk-space/" target="_blank">Technical details</a>',
                  AI1WM_PLUGIN_NAME
                ),
                $meta['uri']
              )
        ...
        if (false === $read_result) {
            if (($meta = stream_get_meta_data($handle))) {
                throw new Ai1wm_Not_Readable_Exception(
                  sprintf(
                    __(
                      'Unable to read file: %s. <a href="https://help.servmask.com/knowledgebase/invalid-file-permissions/" target="_blank">Technical details</a>',
                      AI1WM_PLUGIN_NAME
                    ),
                    $meta['uri']
                  )
    ...
    if (-1 === $seek_result) {
        if (($meta = stream_get_meta_data($handle))) {
            throw new Ai1wm_Not_Seekable_Exception(
              sprintf(
                __(
                  'Unable to seek to offset %d on %s. <a href="https://help.servmask.com/knowledgebase/php-32bit/" target="_blank">Technical details</a>',
                  AI1WM_PLUGIN_NAME
                ),
                $offset,
                $meta['uri']
    ...
    if (false === $tell_result) {
        if (($meta = stream_get_meta_data($handle))) {
            throw new Ai1wm_Not_Tellable_Exception(
              sprintf(
                __(
                  'Unable to get current pointer position of %s. <a href="https://help.servmask.com/knowledgebase/php-32bit/" target="_blank">Technical details</a>',
                  AI1WM_PLUGIN_NAME
                ),
                $meta['uri']
              )
 ...
 * @return boolean
 */
function ai1wm_is_filename_supported($file, $extensions = ['wpress'])
{
...
function ai1wm_verify_secret_key($secret_key)
{
    if ($secret_key !== get_option(AI1WM_SECRET_KEY)) {
        throw new Ai1wm_Not_Valid_Secret_Key_Exception(
          __(
            'Unable to authenticate the secret key. <a href="https://help.servmask.com/knowledgebase/invalid-secret-key/" target="_blank">Technical details</a>',
            AI1WM_PLUGIN_NAME
          )
        );
    }
...
{
    return urldecode(dirname(str_replace(['%2F', '%5C'], '/', urlencode($path))));
}
